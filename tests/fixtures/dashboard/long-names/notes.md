...
Input stresses all three width axes: a 43-char canonical cask token
...
2. **Reasons pay first.** With a 43-char name the reason budget goes negative,
   so the reason column is dropped table-wide (rows keep name + versions +
   label, aligned).
3. **Versions truncate structurally.** The versions column caps at 26 chars;
   ...
   "…" while the " → " arrow is always preserved (`2026.… → 2026.…`), so the
