...
### Changed
- Negative probe cache: when the non-GitHub release-notes chain concludes "nothing" for a package@version, that outcome is remembered for 10 minutes so an immediate re-run skips the full probe chain (up to ~14 URLs with retries per no-notes package — the dominant cost of a fast second run, since failed probes are deliberately never cached). A successful chain clears the entry immediately, a release published mid-window is picked up after expiry, and `--fresh` still forces a full re-probe. Performance memo only — no classification changes.
